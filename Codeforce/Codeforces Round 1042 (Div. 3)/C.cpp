...
The first line contains two integers n and k (1≤n≤2⋅105, 1≤k≤109) — the size of S and the constant, respectively.

The second line contains n integers S1,S2,…,Sn (0≤Si≤109) — the elements in S.

The third line contains n integers T1,T2,…,Tn (0≤Ti≤109) — the elements in T.
...
int main() {
    ...
    while (t--) {
        ...
        vector<long long> sKeys, tKeys;
        sKeys.reserve(n);
        tKeys.reserve(n);

        auto cls = [&](long long x) -> long long {
            long long r = (k == 0 ? x : x % k); // k >= 1 in problem, but safe
            long long rr = (k == 0 ? 0 : (k - r) % k);
            return min(r, rr);
        };

        for (int i = 0; i < n; ++i) {
            long long x; cin >> x;
            sKeys.push_back(k ? cls(x) : 0);
        }
        for (int i = 0; i < n; ++i) {
            long long x; cin >> x;
            tKeys.push_back(k ? cls(x) : 0);
        }

        sort(sKeys.begin(), sKeys.end());
        sort(tKeys.begin(), tKeys.end());

        cout << (sKeys == tKeys ? "YES\n" : "NO\n");
    }
    return 0;
}